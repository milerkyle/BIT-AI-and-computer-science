...
\tikzset{
  inline node/.style = {rectangle,minimum height = 0.8 em,anchor = base},
  inline arrow/.style = {->,>=stealth}
  startstop/.style = {rectangle,rounded corners,text centered},
  process/.style = {rectangle,text centered},
  decision/.style = {diamond,text centered},
...
\end{tabular}
\subsection{浮点数表示}
浮点数表示形式：$X=\pm \textbf{尾数}\times 2^{\textbf{阶}}$
...
\begin{tabular}{c|ccccc|c}
  ...
  & & & 浮点数表示 & & & \\
  \hline
  精度 & 阶符 & 阶码 & \vline & 数符 & 尾数 & 总位数 \\
  \hline
  单精度 & 1位 & 7位 & \vline & 1位 & 23位 & 32位4字节 \\
  双精度 & 1位 & 11位 & \vline & 1位 & 51位 & 64位8字节 \\
  \hline
\end{tabular}
...
\begin{tabular}{ccccccc}
  ...
  \begin{tikzpicture}[baseline=(startstop.base),
    startstop/.style = {rectangle,rounded corners = 9pt,align = center}]
    \node (startstop)[draw,startstop]{开始/终止};
  \end{tikzpicture}
  &
  \begin{tikzpicture}[baseline=(a.base)]\node[draw,trapezium,trapezium left angle = 70,trapezium right angle = 110,text centered](a){输入/输出};\end{tikzpicture}
  &
  \begin{tikzpicture}[baseline=(a.base)]\node[draw,rectangle,text centered](a){处理};\end{tikzpicture}
  &
  \begin{tikzpicture}[baseline=(a.base)]\node[draw,diamond,aspect = 2,text centered](a){判断};\end{tikzpicture}
  &
  \begin{tikzpicture}[]\draw[->,>={Stealth[round,scale = 1.5]}](0,0)--(1,0) ;\end{tikzpicture}
  &
  \begin{tikzpicture}[baseline=(connection.base),connection/.style = {circle,align = center}]\node(connection)[draw,connection]{连接};\end{tikzpicture}
  &
    ...
    nrb/.style={
      ...
      append after command={
        \pgfextra{
          ...
          \draw [-,dotted,thick] (0,0)--(annotation.west);
        }
      }
    }
  ]
...
\item 基本结构：
  \begin{figure}[H]
    ...
    \begin{tikzpicture}[
      ...
      \draw [thick,->,>=stealth] (input.south) -- (process1.north);
      \draw [thick,->,>=stealth] (process1.south) -- (process2.north);
      \draw [thick,->,>=stealth] (process2.south) -- (output.north);
      \draw [thick,->,>=stealth] (output.south) -- (stop.north);
    \end{tikzpicture}
    \caption{顺序结构：输入圆的半径R，输出圆的周长和面积}
  \end{figure}
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}[
      ...
      \node (start)[draw,startstop]{开始};
      \node (input)[draw,io,below = of start]{输入半径$R$};
      \node (decision)[draw,decision,below = of input] {判断$R>=0$};
      ...
      \path (start.south) edge[thick,->,>=stealth] (input.north);
      \path (input.south) edge[thick,->,>=stealth] (decision.north);
      \draw [thick,->,>=stealth] (decision.east) -| (process2.north) node[midway,right]{\tiny 否};
      \path (decision.south) edge[thick,->,>=stealth] node[midway,right]{\tiny 是} (process1.north);
      \path (process1.south) edge[thick,->,>=stealth] (output.north);
      \draw [thick,-] (process2.south) |- ($(process1.south)!0.5!(output.north)$);
      \path (output.south) edge[thick,->,>=stealth] (stop.north);
    \end{tikzpicture}
    \caption{选择结构:输入任意常数R，输出R的绝对值S}
  \end{figure}
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}[
      ...
      \node (decision)[draw,decision,below = of definition2] {判断$I<=K$};
      ...
      \path (start.south) edge[thick,->,>=stealth] (input.north);
      \path (input.south) edge[thick,->,>=stealth] (definition1.north);
      \path (definition1.south) edge[thick,->,>=stealth] (definition2.north);
      \path (definition2.south) edge[thick,->,>=stealth] (decision.north);
      \path (decision.south) edge[thick,->,>=stealth] node[midway,right]{\tiny 是} (process1.north);
      \path (process1.south) edge[thick,->,>=stealth] (process2.north);
      \draw [thick,-] (process2.south) -- ($(process2.south)!0.3!(output.north)$) --++(-2,0) |- ($(definition2.south)!0.5!(decision.north)$);
      \draw [thick,->,>=stealth] (decision.east) --++(.5,0) |- node[midway,right]{\tiny 否} ($(process2.south)!0.7!(output.north)$) -- (output.north);
      \path (output.south) edge[thick,->,>=stealth] (stop.north);
    \end{tikzpicture}
    \caption{循环结构:输入任意自然数K，输出自然数1到K的和}
  \end{figure}
